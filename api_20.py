...
def les_info():
    ...
    url = "https://data.norges-bank.no/api/data/EXR/B.USD+AUD+BDT+BRL+GBP+BGN+DKK+" \
              ...
              "startPeriod={}&endPeriod={}&locale=no".format(start_l, end_l)
    # some code
    history_l = requests.get(url)
    info = history_l.json()
    # base for currencies
...
def get_info():
    ...
    try:
        ...
        if old_date < end_l:
            api_url = "https://data.norges-bank.no/api/data/EXR/B.USD+AUD+BDT" \
                      "+BRL+GBP+BGN+DKK+EUR+PHP+HKD+XDR+INR+IDR+TWI+ISK+JPY+" \
                      "CAD+CNY+HRK+MXN+MMK+NZD+ILS+RON+BYN+TWD+PKR+PLN+RUB+SGD" \
                      "+CHF+SEK+ZAR+KRW+THB+CZK+TRY+HUF.NOK.SP?format=sdmx-json&" \
                      "startPeriod={}&endPeriod={}&locale=no".format(start_l, end_l)
    ...
    # if there is no data stored, the program will fetch data from the api
        ...
        start_l = date.today() + relativedelta(months=-6)
        api_url = "https://data.norges-bank.no/api/data/EXR/B.USD+AUD+BDT+" \
                  "BRL+GBP+BGN+DKK+EUR+PHP+HKD+XDR+INR+IDR+TWI+ISK+JPY+CAD+" \
                  "CNY+HRK+MXN+MMK+NZD+ILS+RON+BYN+TWD+PKR+PLN+RUB+SGD+CHF+" \
                  "SEK+ZAR+KRW+THB+CZK+TRY+HUF.NOK.SP?format=sdmx-json&startPeriod" \
                  "={}&endPeriod={}&locale=no".format(start_l, end_l)
        api_info = requests.get(api_url)
        info = api_info.json()
        cur_dict = find_curencies(info)
        dates = find_dates(info)
...
if __name__ == '__main__':
    usd_url = "https://data.norges-bank.no/api/data/EXR/B.USD.NOK.SP?format=sdmx-json&startPeriod=2019-12-05" \
              "&endPeriod=2021-05-05&locale=no"
    new_url = "https://data.norges-bank.no/api/data/EXR/B.USD+AUD+BDT+BRL+GBP+BGN+DKK+EUR+PHP+HKD" \
              "+XDR+I44+INR+IDR+TWI+ISK+JPY+CAD+CNY+HRK+MXN+MMK+NZD+ILS+RON+BYN+TWD+PKR+PLN+RUB+SGD+" \
              "CHF+SEK+ZAR+KRW+THB+CZK+TRY+HUF.NOK.SP?format=sdmx-json&startPeriod=2019-12-05&endPeriod=" \
              "2021-05-05&locale=no"
    formating = "%Y-%m-%d"
    min_df = get_info()
    store_data(min_df)
